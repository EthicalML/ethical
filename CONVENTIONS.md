...
| Flagship initiative carousel | Homepage 04 bounded snap carousel (teaser and survey card bundled as the single first stop filling the row): shade blocks in the inter-card gap above the track (soft translucent segments, active stretches longer, click travels, follows scroll), vertical chevron compositions at the track edges in the strategy accent (neon hover, line-energise press, dimmed at the ends); enters on the global reveal signal sliding in from the right, then purely user-driven; no entrance under reduced motion; horizontal at every width | `OpenSourceShowcase`, `Reveal`                                                        |

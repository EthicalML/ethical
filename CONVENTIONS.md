...
| Section reveal               | Every `data-reveal` element on every route: fires at 50% visibility, with a 45vh fallback for elements too tall to reach it, an unreachable-threshold check and a page-bottom failsafe; `data-reveal="trigger"` roots get the timing signal (`reveal:visible`) without the fade                                                                                                                                                                                                                                                                                                    | `Reveal`, `BaseLayout`                                                                |
...
| Flagship initiative carousel | Homepage 04 snap carousel (static teaser lead-in, survey card at two-thirds width): infinite wrap via edge clones, position bubbles above the track (active stretches to a pill, click travels, follows scroll), hairline-blade prev/next zones in the section padding (white-scale hover tick, pressed pulse); enters on the global reveal signal sliding in from the right, one 4s auto-advance counted down on the active bubble and flashed on the next blade, cancelled by any interaction; no entrance, countdown or advance under reduced motion; horizontal at every width | `OpenSourceShowcase`, `Reveal`                                                        |

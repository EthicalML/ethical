...
| Policy record timeline      | `/policy/` achievement visual                                                                                                                                                                                                                                                                                                                                   | `PolicyTimeline`, `CanvasEngine`                                                                                                              |
| Policy hero canvas studies  | `/prototypes/policy-hero/?v=1..2`                                                                                                                                                                                                                                                                                                                               | `PhraseTerrain`, `PolicyHeroPolicyCircuit`, `PolicyHeroIso`, `PolicyHeroShared`, `CanvasEngine`                                               |
| Policy record studies       | `/prototypes/policy-record/?v=1..5` preview-rail selection, adoption-diff highlight, position-thread lighting, per-document phrase-terrain reformation, and instrument-timeline strip hover/scroll coupling                                                                                                                                                     | `PolicyRecordPreview`, `PolicyRecordDiff`, `PolicyRecordThreads`, `PolicyRecordTerrain`, `PolicyRecordStrip`, `PhraseTerrain`, `CanvasEngine` |
...
| Policy achievement studies  | `/prototypes/policy-achievements/?v=1..3` three layouts (stack, full-width grid, set-table collage) sharing one card system: top-anchored anatomy, ghost numeral or pixel-cluster surface, inert duds (square, diamond, triangle, plain, accent slab, diamond outline), bottom-up lego falls, and per-block card-displacing neon reveals (sideways or diagonal) | `AchievementNeonRendition`                                                                                                                    |

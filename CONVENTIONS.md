...
| Flagship initiative carousel | Homepage 04 five-card snap carousel (survey card at two-thirds width): enters at 50% visibility sliding in from the right, one 10s auto-nudge to the next card cancelled by any interaction, otherwise user-controlled prev/next only; no entrance or nudge under reduced motion; stacked list below 900px | `OpenSourceShowcase`                                                                  |

...
| Flagship initiative carousel | Homepage 04 bounded snap carousel (static teaser lead-in, survey card at two-thirds width): position bubbles above the track (stretched pills; active stretches longer, click travels, follows scroll), floating glass-disc prev/next hovering at the track edges (white-scale, disabled at the ends); enters on the global reveal signal sliding in from the right, then purely user-driven; no entrance under reduced motion; horizontal at every width | `OpenSourceShowcase`, `Reveal`                                                        |

...
- Build configuration is read in `astro.config.mjs` with Vite's `loadEnv`; `FORM_ENDPOINT` configures form delivery. The URL is base64-encoded at build and inlined into the form's client bundle through a Vite define (`__FORM_TOKEN__`), never into the rendered HTML: the URL cannot be secret on a static site, so this only costs a scraper its grep, and the receiver in `scripts/forms/apps-script.gs` carries the real defences. The receiver replies through a redirect chain that has taken 15 seconds and `mode: no-cors` makes the reply unreadable, so the form confirms after a 2.5 second settle window and relies on `keepalive` to finish delivery.
...
| Section reveal               | Every `data-reveal` element on every route: fires at 25% visibility, with a 45vh fallback for elements too tall to reach it, an unreachable-threshold check and a page-bottom failsafe; `data-reveal="trigger"` roots get the timing signal (`reveal:visible`) without the fade                                                                                                                                                                                                                                                                                                                                                   | `Reveal`, `BaseLayout`                                                                |

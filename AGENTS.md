...
- The reveal pre-hide must never gate the first paint. `BaseLayout`'s inline stylesheet holds `[data-reveal]` at opacity 0, but the reveal module ships in a deferred bundle, so a pre-paint inline script at the foot of `<body>` marks every target starting inside the viewport as revealed, and a `<noscript>` override un-hides everything when there is no script at all. Below-fold reveal motion is unchanged; above the fold the entrance is deliberately gone, because LCP text may not wait on script execution.
- An element whose visible text its own script rewrites at mount carries `data-paint-hold`, because painting the first viewport immediately would otherwise show the server copy and then the script's wipe. The same pre-paint script hides held elements, and the owning script clears the hold (`style.removeProperty('visibility')`) once the text reads what it means it to read, on every exit path including its bail-outs. `visibility` keeps the reserved layout still, and holding from script rather than CSS means a scriptless client — which has no wipe coming — is never held. Hero's typewriter subtitle is the only holder; do not add one for text a user gesture rewrites, which cannot flash on load.

...
| Header logo collapse         | Every route: on page load the header logo holds the two-line wordmark for 1800ms, then collapses its own I, E, A and S into the 2x2 mark over 900ms while the other letters scatter and thin, and rests there. It plays once per full page load, not once per client-side navigation, and does not reverse. The link's height is reserved so the row cannot change height; its width tweens with the mark, so the nav travels left in step. Below 1040px and under reduced motion the mark is rendered statically                                                                                                                 | `SiteLogo`, `SiteHeader`                                                              |

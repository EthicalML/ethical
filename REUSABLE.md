...
- `src/components/CategoryMosaic.astro` (`<category-mosaic>`, `src/shared/canvas/CategoryMosaic.ts`). The twinkling catalogue mosaic the header's nav preview draws, as a standalone mount. `set` chooses the tile set: `production-ml` (24 categories), `ai-guidelines` (regulation themes and economic areas) or `agentic-systems` (seven agentic tooling categories, painted in the violet tint). Adding a set means adding a tile list and registering it in `TILE_SETS`, optionally with a `MosaicTint` so sibling catalogues read apart.

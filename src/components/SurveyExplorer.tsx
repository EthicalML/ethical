...
export default function SurveyExplorer() {
  ...
  const responseCount=year==='a'?set.n2024:year==='b'?set.n2025:`${set.n2024} / ${set.n2025}`;
  ...
  return <div class="survey-island" data-survey-card>
    ...
    <div class="survey-focus"><div><span>SELECTED</span><strong>{selected.label}</strong></div><div><span>SHARE OF RESPONDENTS</span><strong>{selected.v}% of respondents</strong></div><div><span>YEAR ON YEAR</span><strong>{selected.d>0?'+':''}{selected.d} pts vs 2024</strong></div>{selected.note&&<p>{selected.note}</p>}</div>
  </div>;
}

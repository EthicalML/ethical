...
quickstart:
  ...
  helm:
    - command: helm repo add kaos https://axsaucedo.github.io/kaos/charts
      output: Repository "kaos" added
    - command: helm install kaos kaos/kaos-operator -n kaos-system --create-namespace
      output: STATUS deployed
    - command: kubectl apply -f my-agent.yaml
      output: agent.kaos.ai/my-agent created
...
<QuickstartTerminal cli={frontmatter.quickstart.cli} helm={frontmatter.quickstart.helm} />
...
<MemoryTiers storage="LOCAL: Chroma + SQLite on one volume · EXTERNAL: pgvector + Postgres, 2 replicas" />

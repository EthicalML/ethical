...
10. Suggestion, not a requirement. A named-individual opinion or analysis piece is welcome when a strong one exists and it argues something rather than venting. Roughly 3 issues in 5 carry one, so an issue without one is entirely normal. Never displace a stronger candidate to fill this slot.
...
13. Suggestion, not a requirement. Weight toward LLMs, agents and ML security when ranking the pool, and deprioritise generic MLOps process and policy. This is a tendency across issues rather than a per-issue composition check: an issue that lands infra-heavy or research-heavy is fine.

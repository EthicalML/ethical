...
```sh
npm run build -- --base=/keynotes/signals-2026/ --outDir=tmp/keynote-package
```

Replace this repository's `public/keynotes/signals-2026/` with that output in a new website worktree. Copy the source project's current `PREFLIGHT.md` into the package, and `node_modules/three/LICENSE` as `THREE-LICENSE.txt`. Do not edit hashed bundles directly.

Keep Vite's `/keynotes/signals-2026/` base and the runtime `import.meta.env.BASE_URL` asset references: root-relative portrait, QR, or checklist paths will break on the published URL. All presentation assets are local; no server or new deployment workflow is required. Google Analytics is an optional external request on the production hostname only, excluded from localhost and presenter windows.

## Verification

Run the website's lint, ratchet and build checks. Serve the resulting `dist/` and verify `/keynotes/signals-2026/`, including all 41 stops, deep-link reloads, presenter notes at the final stop, clicker controls, and desktop/mobile layouts. The source project's `scripts/verify-clicker.mjs` accepts `KEYNOTE_URL` for checking the packaged route.
...
The static HTML includes Open Graph and X large-image card metadata for the first-slide image at `/keynotes/signals-2026/keynote-preview-20260910.jpg` (1200 by 675). Crawlers do not need to execute JavaScript. Regenerate it from the source project's local preview using `node scripts/capture-sharing-preview.mjs`; version the image filename and metadata together when changing the picture. Cached previews on sharing platforms may need a fresh scrape.
...
`public/keynotes/ai-enterprise-2026/` contains the 24-stop enterprise presentation, including the final gathering galaxy and live-deck QR. Editable source is in the `enterprise-ai-2026/site/` directory of the KAOS docs repository. Build with `npm run build -- --base=/keynotes/ai-enterprise-2026/`; the production environment sets the live deck URL. The portrait enters with its slide without a second reveal reset. Sharing metadata uses the opening-slide `preview.jpg`.

The former `/keynote/` address redirects to `/keynotes/signals-2026/`. Both talks use the existing Institute analytics property, disabled on localhost and presenter views. Browser verification intercepts analytics requests.

...
Run the website's lint, ratchet and build checks. Serve the resulting `dist/` and verify `/keynote/`, including all 38 stops, deep-link reloads, presenter notes at the final stop, clicker controls, and desktop/mobile layouts. The source project's `scripts/verify-clicker.mjs` accepts `KEYNOTE_URL` for checking the packaged route.

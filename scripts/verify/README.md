...
`verify-motion` is the one gate that watches the journey rather than the destination. A real bug survived on `master` through every branch and was found by accident: navigating into a principle played its view transition 26px too low and snapped up on completion. Screenshots could not see it — the page was never in the wrong place, the _snapshot_ was. At the time, `.principle-prose` was both a `[data-reveal]` element and the named `principle-page` transition group, and the reveal settle was deferred by `requestAnimationFrame`, which runs after the browser photographs `::view-transition-new`. Fixed in `c38ef92` by settling in a microtask instead. The named group now wraps the complete principle body, with `.principle-prose` retained as the tracked reveal target inside it.

...
`verify-motion` is the one gate that watches the journey rather than the destination: screenshots cannot catch a transition that plays from the wrong position and snaps into place on completion, because the page is never wrong — only the _snapshot_ is. The constraint it protects: any settle applied to an element inside a named transition group must complete before the browser photographs `::view-transition-new`, which means settling in a microtask, never in `requestAnimationFrame` (rAF runs after the photograph). On principle pages the `principle-page` group wraps the complete principle body, with `.principle-prose` as the tracked reveal target inside it.

The gate clicks a real link (never `goto`), then samples every animation frame for two seconds after the click: the scroll offset, and each tracked element's `getBoundingClientRect()`. A reveal primer that leaks into the snapshot is a transform on the real element (e.g. `translateY(26px)`), and `getBoundingClientRect` sees transforms, so a bad frame is measurable even though the pseudo-element is not.

**The invariant is that arrival happens in a single motion.** From `astro:after-swap` onwards, every tracked element's document-space position — `scrollY + rect.top`, which cancels the router's own scroll — must already be its resting position. A trajectory that visits a position it later corrects away from by more than 2px is two motions. The resting value is the median of the window's last samples, and the assertion is stated against it rather than against "the first plateau" deliberately: a bad frame can be a single sample, too short to form a plateau of its own, and a plateau-relative rule would score it as the settle it corrects into. Principle arrivals additionally assert that the final scroll offset is the page top, so a stable but misplaced mid-text landing cannot pass. The 2px tolerance absorbs sub-pixel settle and layout rounding; it is not somewhere to hide a jump.
...
- Hero canvas playback: asserts the homepage hero cycle paints while visible, stops when scrolled away, does not restart when the tab is hidden while already paused, and resumes on return. The offscreen leg waits for the paint count to stop moving before measuring, because the pause arrives on an `IntersectionObserver` callback and the frames still in flight when the scroll lands are not a defect; the steady-state budget after that is zero. The drawer's desktop-resize leg resizes past the 1040px nav-collapse breakpoint, not to some width that merely feels wide: below it the drawer is correctly still open, and the toggle's state is then out of step with what the rest of the sequence expects.
...
- Mobile checks: asserts the drawer accordions, JOIN access, Escape/focus/scroll-lock handling, 44px touch targets including the drawer's theme toggle (measured with the drawer open, because below the 1040px nav-collapse breakpoint the desktop pill is hidden and the drawer copy is not laid out until it opens), stacked form/footer, responsive evidence/phase grids, non-sticky principles detail, capped hero canvas, and contained survey tabs.
